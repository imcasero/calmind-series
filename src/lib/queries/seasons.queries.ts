...
export async function getActiveSeasonWithSplit(): Promise<SeasonWithActiveSplit | null> {
  ...
  if (!result.success) {
    console.error('[getActiveSeasonWithSplit] Validation error:', result.error);
    return null;
  }
 ...
 */
export async function getAllSeasonsWithSplits(): Promise<SeasonWithSplits[]> {
  'use cache';
  cacheLife('hours');
  cacheTag('seasons');
 ...
 */
export async function getSeasonWithSplits(
  seasonId: string,
): Promise<SeasonWithSplits | null> {
  'use cache';
  cacheLife('hours');
  cacheTag('seasons');
  ...
  if (error || !data) {
    console.error('[getSeasonWithSplits] Error:', error?.message ?? 'No data');
    return null;
  }
 ...
 */
export async function getArchiveSplitParams(): Promise<
  Array<{ season: string; split: string }>
> {
  'use cache';
  cacheLife('days');
  cacheTag('seasons');
 ...
 */
export async function getSplitByNames(
  seasonName: string,
  splitName: string,
